...
from schemas.booking import Booking, BookingUpdate
from sqlalchemy.orm import Session


def get_bookings(db: Session, user_id: int):
    return db.query(DBBooking).filter(DBBooking.owner_id == user_id).all()
...
def update_booking(db: Session, booking: BookingUpdate, id: int, user_id: int):
